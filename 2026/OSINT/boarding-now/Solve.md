...
* **Target Date:** travel date from the julian calendar date in the barcode
...
4. **Timeline Scrape:** Pinpoint the stream archive matching the travel date and find the first timestamp stamped on the video player or overlay feed.
...
3. **Finding the Timestamp:** Locate the archived stream broadcast for travel date from the julian calendar date in the barcode. Scrubbing directly to the beginning of that broadcast block reveals the first operational flight movement captured on the layout overlay happens exactly at **05:43:46 UTC**.
